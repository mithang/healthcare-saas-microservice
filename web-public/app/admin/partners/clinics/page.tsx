        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 animate-fade-in">
                ...
                <p className="text-gray-500 text-center mb-6">
                    Bạn có chắc chắn muốn xóa phòng khám <span className="font-bold text-gray-800">"{itemName}"</span> không? Hành động này không thể hoàn tác.
                </p>
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Tổng: {filteredData.length} phòng khám</p>
                </div>
                <Link href="/admin/partners/clinics/create" className="bg-primary text-white font-bold px-6 py-3 rounded-xl hover:bg-primary-dark transition shadow-lg shadow-primary/25 flex items-center gap-2">
                    ...
                    <table className="w-full">
                        <thead className="bg-gray-50/50 border-b border-gray-100">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Chuyên khoa</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {paginatedData.map((clinic: any) => (
                                <tr key={clinic.id} className="hover:bg-gray-50/50 transition-colors">
                                    <td className="px-6 py-4 font-medium text-gray-900">{clinic.name}</td>
                                    <td className="px-6 py-4 text-sm text-gray-600 max-w-xs truncate" title={clinic.address}>{clinic.address}</td>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex gap-1 flex-wrap">
                                            ...
                                            {clinic.specialties?.length > 2 && (
                                                <span className="px-2.5 py-1 bg-gray-50 text-gray-600 text-xs font-medium rounded-full border border-gray-100">+{clinic.specialties.length - 2}</span>
                                            )}
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex justify-end gap-3">
                                            <Link
                                                href={`/admin/partners/clinics/${clinic.id}/edit`}
                                            ...
                                            <button
                                                onClick={() => openDeleteModal(clinic.id, clinic.name)}
            ...
            {/* Use Pagination Controls always if there is data, or check totalPages > 0 */}
            {totalPages > 0 && (
