                    ...
                    <table className="w-full">
                        <thead className="bg-gray-50/50 border-b border-gray-100">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Giường</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {paginatedData.map((hospital: any) => (
                                <tr key={hospital.id} className="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-600 max-w-xs truncate" title={hospital.address}>{hospital.address}</td>
                                    <td className="px-6 py-4 text-sm text-gray-600">{hospital.phone}</td>
                                    <td className="px-6 py-4">
                                        <div className="flex gap-1 flex-wrap">
                                            {hospital.departments?.slice(0, 2).map((d: string, i: number) => (
                                            ...
                                            {hospital.departments?.length > 2 && (
                                                <span className="px-2.5 py-1 bg-gray-50 text-gray-600 text-xs font-medium rounded-full border border-gray-100">+{hospital.departments.length - 2}</span>
                                            )}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-sm text-gray-600">{hospital.beds || 0}</td>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex justify-end gap-3">
                                            <Link href={`/admin/partners/hospitals/${hospital.id}/edit`} className="p-2 text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-lg transition-colors group">
                                                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </Link>
                                            <button onClick={() => openDeleteModal(hospital.id, hospital.name)} className="p-2 text-red-600 hover:text-red-800 hover:bg-red-50 rounded-lg transition-colors group">
                                                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
